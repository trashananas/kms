...
2. Добавьте переменную `YANDEX_MAPS_API_KEY` со значением вашего ключа от сервиса **«JavaScript API и HTTP Геокодер»**.
3. Переменная используется сервером как прокси для геокодера; клиенту ключ не передаётся.
4. Файл `vercel.json` в корне проекта уже настроен для деплоя Express-приложения.

> Шаблон `.env.example` показывает, какие переменные нужны для локального запуска (скопируйте в `.env` и заполните свои значения).
